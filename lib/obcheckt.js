...
function _validate(obj, format, key, callback) {
  ...
  if (format === null) {
    return check(
      obj == null
    , util.format('Expected %s to not exist, but found', key, obj)
    , callback
    )
  }
  ...
  if (Array.isArray(format)) {
    ...
    , util.format('Expected %s to be an Array, but found', key, obj)
    , callback
    )) {
      return false
    }
  ...
  if (typeof format === 'function') {
    if (typeof obj === 'object') {
      ...
      , util.format('Expected %s to be an instance of %s, but found', key, format.name || format, obj)
      , callback
      )
    } else {
      return check(
  ...
  , util.format('Expected %s to be of type "%s", but found', key, typeof format, obj)
  , callback
  )) {
    return false
  }
  ...
  if (typeof format !== 'object') {
    return check(
      obj == format
    , util.format('Expected %s to equal', format, ', but found', obj)
    , callback
    )
  }
  ...
  if (!check(
    obj
  , util.format('Expected %s to be an Object, but found', key, obj)
  , callback
  )) {
    return false
  }
  ...
  return Object.keys(format).every(function (subkey) {
    ...
    if (format[subkey] === Undefined) {
      if (!check(
        obj[subkey] == null
      , util.format('Expected "%s.%s" to not exist', key, subkey)
      , callback
      )) {
        return false
      }
